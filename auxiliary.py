...
def neighbors(vectors, T): #Weight vector's, Number of neighbors 
    ...
    while i < len(vectors): #Going through all the vectors
        ...
        distance = [] #Distance's array for the current vector
        ...
        for vector in vectors: 
            if vector == cVector: 
                ...
                while j < len(vector): #That's the inside of the sqrt
                    ...
                    j = j + 1
                squareRoot = sqrt(sum)
                distance.append(squareRoot)
        
        distance.sort() #We make an ascend order on the list
        neighbors.append(distance[0:T]) #We only choose the T better vectors of the array
...
def differentialEvolution(poblation,functions,F,GR):
    #F -> Mutation rate [0,2]
    ...
    while l < len(f):
        tfit = f[l]
        xfit = functions[l]
        if(tfit[0] < xfit[0] or tfit[1] < xfit[1]):
...
differentialEvolution(p,f,0.3,0.3)